...
**プレースホルダーのブランド導線**:
- マスコット画像全体を `kitepon.dev` への新規タブリンクにする
- 右下にはRootSitePromotionの承認済みprimary wordmarkを無改変・縮小のみで置く
- URLは固定campaign `utm_source=nope&utm_medium=chrome_extension&utm_campaign=nope-brand-link&utm_content=blocked-placeholder` を使い、ユーザー・閲覧商品・ブロック対象を含めない
- 解除ボタンは独立した操作領域とし、ブランドリンクのクリックへ伝播させない
