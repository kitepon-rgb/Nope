...
**確定した実測事実（shiho 2026-08-11）**: 検索結果5件中、handle 形式 `/@xxx` が3件・channelId 形式 `/channel/UC...` が2件。同一チャンネルが両形式で出るかは未確認（ログイン状態でのホーム画面が未実測）。
...
**この決定の結果として生じる既知の限界**:  
同一チャンネルが検索結果で `/@handle` 形式のカードと `/channel/UC...` 形式のカードの両方で出た場合、片方のみでブロックを登録すると、もう片方のカードにはブロックが効かない。
