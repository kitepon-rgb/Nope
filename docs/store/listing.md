...
|---|--------|--------------|------------------------|
| 1 | 検索結果ページ：ブロック済みストアの商品が猫プレースホルダーに置き換わっている（既定表示） | `docs/evidence/r2-placeholder-visible.png` | ブロックしたストアの商品は、控えめな猫プレースホルダーに自動的に置き換わります |
| 2 | 商品ページ：「このストアをブロック」ボタン | `docs/evidence/t3-button-injected.png` / `t3-blocked.png`（1280x800での撮り直しが望ましい。r2実測時は商品ページを対象にしなかったため未更新） | 商品ページの「このストアをブロック」ボタンでワンクリック登録 |
| 3 | ポップアップ：ブロック中ストア一覧・表示モード切替UI | `docs/evidence/r2-popup-closeup.png`（実サイズ360x420でのクローズアップ。1280x800キャンバスへの合成配置は未実施——必要なら別途デザイン作業として行う） | ポップアップでブロック中のストアを確認・削除、表示モードもいつでも切替できます |
| 4 | 完全非表示（collapse）モードに切替後の検索結果（後続カードが前へ詰まる） | `docs/evidence/r2-collapse-mode.png` | 完全に消して空間を詰める表示にも切替可能です |
| 5 | 参考：ブロック前の通常の検索結果（Before比較用） | `docs/evidence/r2-store-search-before-block.png` | （通常のAliExpress検索結果。1〜4との対比用。5枚目として掲載するか、1枚目の前段として使うかは掲載時に判断） |
...
**画像サイズについて（要申し送り）**: design memoは全画像1280x800を求めているが、実際にr2で撮影した画像のうち検索結果ページの3枚（1・4・5番）は1280x800のviewportで撮影済みで要件を満たす。popup（3番）は実際のpopupウィンドウの自然なサイズ（360x420）でクローズアップ撮影しており、1280x800ではない。ストア提出時にそのまま使うか、1280x800キャンバスへ配置し直す編集が要るかは r7-submit で判断すること。
