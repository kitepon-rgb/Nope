...
- オーナー受入結果（2026-08-11）:
  ...
  - YouTube検索結果でのチャンネルブロックとブロック後表示: **成功**
  - YouTube検索結果でのブロック解除: **成功**

この結果でフローティングUI修正そのものは受入済み。工程全体は、検索面、ホームと検索の共有リスト、SPA遷移・追加カード、視聴ページ関連動画の非介入を順に確認してから完了とする。

## オーナー実Chrome受入（H）— 最終合格

検証日: 2026-08-12 / 実行環境: オーナーのログイン済み通常Chrome

ホームと検索をまたぐ共有リストの確認中、押したカードとは異なるチャンネルがブラックリストへ登録される欠陥を発見した。真因は、YouTubeが同じ`ytd-rich-item-renderer`要素の内容を別チャンネルへ差し替える一方、旧実装が要素identityだけを根拠に古い発信元情報を再利用していたことだった。

- 修正commit: `2e99adf`（クリック位置のカード再同定、現DOMの発信元再解決、発信元消失時のfail closed、旧状態の清掃）
- 変更直結テスト: **3件 pass / 0 fail**
- 関連テスト: **75件 pass / 0 fail**
- 全テスト: **214件 pass / 0 fail**
- 円卓再監査: Mashiroが最新差分全体を監査し、要修正なし・受理推奨（room seq 165）
- 再梱包後ZIP SHA-256: `81d6297f0c71e5f3f8d9858fc6b5e1f6a1259bae74b3a7f7ff17770c0ecb60f8`
- `src/`と`dist/chromeblocker-v2.0.0-unpacked/src/`の差分: **なし**

最終オーナー受入結果:

1. ホームの登録、ブロック、高さ維持、解除: **合格**
2. 検索の登録、ブロック、解除: **合格**
3. ホームと検索をまたぐ共有リスト: **合格**
4. SPA遷移と追加カードへの追従: **合格**
5. 視聴ページ関連動画へのNope UI・フィルタ非介入: **合格**
6. 押したカードとブラックリストへ登録されたチャンネルの一致: **合格**

オーナー最終確認: 「トップでブロックして検索画面でもブロックされ、解除も成功し、ターゲットのチャンネルも正しかった」。以上により`yt-package-smoke`のH条件を完了とする。
